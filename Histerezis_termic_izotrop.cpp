...
int main(void)
{
	...
	char cale_out[150] = "D:\\Simulari\\Spin\\Elastic\\Rezultate\\Histerezis_temp\\hexagon";
	...
	sprintf(fisier_out1, "%s%d\\tau_%.0f\\k_elastic_%.0f\\nHS.dat", cale_out, DIM, tau, k_elastic);
	ofstream fout1(fisier_out1);

	nHS = DIM;
	int step;

	for (T = 200; T >= 20; T -= 0.1)
	{
		fout1 << T << ' ' << nHS / DIM << '\n';
		cout << T << ' ' << nHS / DIM << '\n';
		...
		{
			sprintf(fisier_out2, "%s%d\\tau_%.0f\\k_elastic_%.0f\\T%.1f_nHS%f.dat", cale_out, DIM, tau, k_elastic, T, nHS / DIM);
	...
	for (; T <= 450; T += 0.1)
	{
		...
		if (double(nHS / DIM) < 0.7 && double(nHS / DIM) > 0.6)
		{
			sprintf(fisier_out2, "%s%d\\tau_%.0f\\k_elastic_%.0f\\rev_T%.1f_nHS%f.dat", cale_out, DIM, tau, k_elastic, T, nHS / DIM);
			ofstream fout2(fisier_out2);
			for (int i = 0; i < DIM; i++)
			{
				fout2 << sol[4 * i] << ' ' << sol[4 * i + 2] << ' ' << r[i] << '\n';
